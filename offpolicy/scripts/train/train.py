...
def make_train_env(all_args):
    def get_env_fn(rank):
        def init_env():
            if all_args.env_name == "two_robots":
                # env = Env(all_args)
...
def parse_args(args, parser):
    parser.add_argument('--scenario_name', type=str,
                        default='2_robots', help="Which scenario to run on")
...
def qmix_main(args):
    ...
    # choose algo
    if all_args.algorithm_name in ["qmix"]:
        from offpolicy.runner.rnn.sumo_runner import SUMORunner as Runner
        assert all_args.n_rollout_threads == 1, (
            "only support 1 env in recurrent version.")
        eval_env = env
    elif all_args.algorithm_name in ["mqmix"]:
        from offpolicy.runner.mlp.sumo_runner import SUMORunner as Runner
